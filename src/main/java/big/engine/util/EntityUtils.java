...
public class EntityUtils {
    ...
    public static Vec2d getRandomSpawnPosition(int team){
        ...
        for(int i=0;i<500;i++){
            ...
            if(cs.world.getBlockState(Vec2i.ofFloor(pos)).getTeam()==team){
                return pos;
            }
    ...
    public static Vec2d getVisitorSpawnPosition(){
        ...
        for(int i=0;i<700;i++){
            ...
            BlockState state=cs.world.getBlockState(Vec2i.ofFloor(pos));
        ...
        for(int i=0;i<7;i++){
            Vec2d pos=Util.randomInBox(cs.borderBox);
            if(cs.world.getBlockState(Vec2i.ofFloor(pos)).getTeam()>=0){
                continue;
            }
            if(EntityUtils.isInsideWall(new Box(pos,1.5,1.5))){
                continue;
    ...
    public static void updateCollision(Entity entity, Predicate<Entity> neverCheck,Predicate<Entity> check,AfterCheckTask<Entity> task,int r,boolean instantStop){
        Vec2i pos=entity.getChunkPos();
        for(int x=-r;x<=r;x++){
            for(int y=-r;y<=r;y++){
                Vec2i pos2=pos.add(x,y);
    ...
    public static boolean sl(Vec2i pos){
        return !cs.world.getBlockState(pos.add(-1,0)).getBlock().solid;
    }
    public static boolean sr(Vec2i pos){
        return !cs.world.getBlockState(pos.add(1,0)).getBlock().solid;
    }
    public static boolean st(Vec2i pos){
        return  !cs.world.getBlockState(pos.add(0,1)).getBlock().solid;
    }
    public static boolean sb(Vec2i pos){
        return !cs.world.getBlockState(pos.add(0,-1)).getBlock().solid;
    }

...
import nbt.visitor.NbtOrderedStringFormatter;
import nbt.visitor.NbtTextFormatter;
...
public final class NbtHelper {
	private static final Comparator<NbtList> BLOCK_POS_COMPARATOR = Comparator.comparingInt((nbt) -> {
		return nbt.getInt(1);
	...
	private static final Comparator<NbtList> ENTITY_POS_COMPARATOR = Comparator.comparingDouble((nbt) -> {
		...
		if (standard == subject) {
			...
			if (standard instanceof NbtCompound) {
				...
				if (nbtCompound2.getSize() < nbtCompound.getSize()) {
					...
					var6 = nbtCompound.getKeys().iterator();
					...
					} while(matches(nbtElement, nbtCompound2.get(string), ignoreListOrder));
	...
	public static StringBuilder appendFormattedString(StringBuilder stringBuilder, NbtElement nbt, int depth, boolean withArrayContents) {
		...
		switch (nbt.getType()) {
			...
			case 9:
				...
				string = i == 0 ? "undefined" : NbtTypes.byId(i).getCommandFeedbackName();
			...
			case 10:
				...
				i = list.stream().mapToInt(String::length).max().orElse(0);
	...
	static NbtCompound toNbtProviderFormat(NbtCompound compound) {
		...
		NbtList nbtList2 = (NbtList)var10000.map(NbtCompound.class::cast).map(NbtHelper::toNbtProviderFormattedPalette).map(NbtString::of).collect(Collectors.toCollection(NbtList::new));
		...
		if (compound.contains("entities", NbtElement.LIST_TYPE)) {
			...
			var10000 = nbtList3.stream();
			Objects.requireNonNull(NbtCompound.class);
			nbtList4 = (NbtList)var10000.map(NbtCompound.class::cast).sorted(Comparator.comparing((nbt) -> {
		...
		Objects.requireNonNull(NbtCompound.class);
		nbtList3 = (NbtList)var10000.map(NbtCompound.class::cast).sorted(Comparator.comparing((nbt) -> {
			return nbt.getList("pos", NbtElement.INT_TYPE);
		}, BLOCK_POS_COMPARATOR)).peek((nbt) -> {
			nbt.putString("state", nbtList2.getString(nbt.getInt("state")));
	...
	static NbtCompound fromNbtProviderFormat(NbtCompound compound) {
		...
		Map<String, NbtElement> map = (Map)var10000.map(NbtString.class::cast).map(NbtString::asString).collect(ImmutableMap.toImmutableMap(Function.identity(), NbtHelper::fromNbtProviderFormattedPalette));
		if (compound.contains("palettes", NbtElement.LIST_TYPE)) {
			...
			compound.put("palettes", (NbtElement)var10002.map(NbtCompound.class::cast).map((nbt) -> {
				...
				return (NbtList)var10000.map(nbt::getString).map(NbtHelper::fromNbtProviderFormattedPalette).collect(Collectors.toCollection(nbt.NbtList::new));
			}).collect(Collectors.toCollection(NbtList::new)));
	...
	static String toNbtProviderFormattedPalette(NbtCompound compound) {
		...
		if (compound.contains("Properties", NbtElement.COMPOUND_TYPE)) {
			...
			String string = (String)nbtCompound.getKeys().stream().sorted().map((key) -> {
				return key + ":" + nbtCompound.get(key).asString();
